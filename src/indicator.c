...
static GtkTreeModel * create_model (void)
{
	...
	for (i = 0; i < G_N_ELEMENTS (data); i++)
	{
		...
		gtk_list_store_set (store, &iter,
				COLUMN_ACTIVE_ICON, i == 0 ? active_freq_pixbuf : NULL,
				COLUMN_MUTED, data[i].muted,
				COLUMN_FREQ, data[i].frequency,
				COLUMN_LAST_ACTIVE, data[i].last_active,
...
static void muted_toggled (GtkCellRendererToggle *cell,
               gchar                 *path_str,
               gpointer               data)
{
...
static void add_columns (GtkTreeView *treeview)
{
	...
	g_signal_connect (renderer, "toggled",
			G_CALLBACK (muted_toggled), model);

	column = gtk_tree_view_column_new_with_attributes ("mute",
			renderer,
			"active", COLUMN_MUTED,
			NULL);
	...
	renderer = gtk_cell_renderer_text_new ();
	column = gtk_tree_view_column_new_with_attributes ("last heard",
			renderer,
			"text",
			COLUMN_LAST_ACTIVE,
			NULL);
...
int main( int argc, char** argv )
{
	...
	ci = app_indicator_new( "sdr-scanner", ICON_SCANNING, APP_INDICATOR_CATEGORY_APPLICATION_STATUS );
	...
	//app_indicator_set_icon( ci, "/usr/share/icons/ubuntu-mono-dark/status/24/nm-signal-100.svg" ); //"/home/alex/gh/alexcb/sdr-scanner/radio.svg" );
