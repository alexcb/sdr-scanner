...
static gboolean
custom_list_get_iter( GtkTreeModel* tree_model, GtkTreeIter* iter, GtkTreePath* path );

static GtkTreePath* custom_list_get_path( GtkTreeModel* tree_model, GtkTreeIter* iter );

static void
custom_list_get_value( GtkTreeModel* tree_model, GtkTreeIter* iter, gint column, GValue* value );

static gboolean custom_list_iter_next( GtkTreeModel* tree_model, GtkTreeIter* iter );

static gboolean
custom_list_iter_children( GtkTreeModel* tree_model, GtkTreeIter* iter, GtkTreeIter* parent );
...
static gboolean
custom_list_iter_parent( GtkTreeModel* tree_model, GtkTreeIter* iter, GtkTreeIter* child );
...
GType custom_list_get_type( void )
{
	...
	if( custom_list_type == 0 ) {
		static const GTypeInfo custom_list_info = {sizeof( CustomListClass ),
												   NULL, /* base_init */
												   NULL, /* base_finalize */
		...
		custom_list_type =
			g_type_register_static( G_TYPE_OBJECT, "CustomList", &custom_list_info, (GTypeFlags)0 );
...
static void custom_list_init( CustomList* custom_list )
{
	...
	custom_list->column_types[4] = G_TYPE_INT; // CUSTOM_LIST_COL_LAST_ACTIVE,

	g_assert( CUSTOM_LIST_N_COLUMNS == 5 );

	custom_list->num_rows = 0;
	custom_list->rows = NULL;

	custom_list->stamp =
		g_random_int(); /* Random int to check whether an iter belongs to our model */
}
...
custom_list_get_iter( GtkTreeModel* tree_model, GtkTreeIter* iter, GtkTreePath* path )
{
	...
	g_assert( depth ==
			  1 ); /* depth 1 = top level; a list only has top level nodes and no children */
...
custom_list_get_value( GtkTreeModel* tree_model, GtkTreeIter* iter, gint column, GValue* value )
{
	...
	case CUSTOM_LIST_COL_MUTED:
		g_value_set_int( value, record->muted ? 1 : 0 );
		break;

	case CUSTOM_LIST_COL_FREQUENCY:
		g_value_set_int( value, record->frequency );
		break;
	...
	case CUSTOM_LIST_COL_LAST_ACTIVE:
		g_value_set_int( value, record->frequency );
		break;

	default:
		printf( "unsupported column %d\n", column );
		assert( 0 );
	}
}
...
void custom_list_append_record( CustomList* custom_list, const char* name, int frequency )
{
	GtkTreeIter iter;
	GtkTreePath* path;
